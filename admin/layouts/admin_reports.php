...
<table class="table table-bordered table-hover">
	...
	$from=$date.' 00:00:00';
	$to=$date.' 23:59:59';
	$sql = 'SELECT u1.id,u1.name, p.projectname, r1.description, r1.reportdate FROM report r1 INNER JOIN user u1 ON r1.userid=u1.id INNER JOIN project p on r1.reporttitle=p.id where reportdate between"'.$from.'" and "'.$to.'"';
	if($result=mysqli_query($link,$sql)){
		if(mysqli_num_rows($result)>0){
			$table='';
			while($row=mysqli_fetch_array($result)){
				$table.='<tr><td class="id'.$row['id'].'">'.$row['name'].'</td><td class="report_title'.$row['id'].'">'.$row['projectname'].'</td><td class="report_description'.$row['id'].'">'.$row['description'].'</td><td class="report_date'.$row['id'].'">'.date('d-M-Y', strtotime($row['reportdate'])).'</td></tr>';
			}
			echo $table;
		}
		else{
