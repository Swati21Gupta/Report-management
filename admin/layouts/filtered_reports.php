...
<script>
  $(function(){
    $('body').on('click','.pages1',function(){
      ...
      var dataString = {strlimit:strlimit,date:date};
      $.ajax({
        type:"POST",
        data:dataString,
        dataType:"json",
        ...
        success:function(result){
          ...
          for(i=0;i<result.length;i++){
            table +='<tr><td class="id'+result[i].id+'">'+result[i].name+'</td><td class="report_title'+result[i].id+'">'+result[i].projectname+'</td><td class="report_description'+result[i].id+'">'+result[i].description+'</td><td class="report_date'+result[i].id+'">'+result[i].reportdate+'</td></tr>';
          }
...
<table class="table table-bordered table-hover data_table">
  <tbody>
    ...
    if($filter_name=="" && $filter_date==""){
      ...
      $sql = 'SELECT u1.id,u1.name, p.projectname, r1.description, r1.reportdate FROM report r1 INNER JOIN  user u1 ON r1.userid=u1.id INNER JOIN project p on r1.reporttitle=p.id where 1=1 '.$cond.' LIMIT 5';
      ...
      if($result=mysqli_query($link,$sql)){
        ...
        if(mysqli_num_rows($result)>0){
          ...
          while($row=mysqli_fetch_array($result)){
            ...
            <td class="report_title'.$row['id'].'">'.$row['projectname'].'</td>
